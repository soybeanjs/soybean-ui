...
Source URL: https://veanui.com/components/calendar
Markdown URL: https://veanui.com/components/calendar.md
Category: Forms
...
- `CalendarCompact` (headless) — data-driven composition of `CalendarRoot` + header (prev/heading/next) + one grid per month (grid head/body rows of `CalendarCellTrigger`); import from `@vean/aria/calendar` for unstyled usage
