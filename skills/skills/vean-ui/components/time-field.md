...
Source URL: https://veanui.com/components/time-field
Markdown URL: https://veanui.com/components/time-field.md
Category: Forms
...
- `TimeFieldCompact` (headless) — data-driven composition of `TimeFieldRoot` + one `TimeFieldInput` per segment, plus `leading`/`trailing` slots; import from `@vean/aria/time-field` for unstyled usage
