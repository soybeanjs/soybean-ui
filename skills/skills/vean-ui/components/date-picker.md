...
Source URL: https://veanui.com/components/date-picker
Markdown URL: https://veanui.com/components/date-picker.md
Category: Forms
...
- `DatePickerCompact` (headless) — data-driven composition of `DateFieldCompact` + `PopoverCompact` (calendar-icon trigger) that exposes the popup state and `calendarProps` through its default slot; import from `@vean/aria/date-picker` for unstyled usage
