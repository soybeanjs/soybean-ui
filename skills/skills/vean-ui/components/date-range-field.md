...
Source URL: https://veanui.com/components/date-range-field
Markdown URL: https://veanui.com/components/date-range-field.md
Category: Forms
...
- `DateRangeFieldCompact` (headless) — data-driven composition of `DateRangeFieldRoot` + per-segment `DateRangeFieldInput` for both groups, plus `leading`/`trailing`/`separator` slots; import from `@vean/aria/date-range-field` for unstyled usage
