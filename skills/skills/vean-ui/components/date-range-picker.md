...
Source URL: https://veanui.com/components/date-range-picker
Markdown URL: https://veanui.com/components/date-range-picker.md
Category: Forms
...
- `DateRangePickerCompact` (headless) — data-driven composition of `DateRangeFieldCompact` + `PopoverCompact` (calendar-icon trigger) that exposes the popup state and `calendarRangeProps` through its default slot; import from `@vean/aria/date-range-picker` for unstyled usage
