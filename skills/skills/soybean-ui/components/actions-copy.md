...
Usage examples for actions-copy are rendered on the site.
...
Interactive demos for actions-copy are rendered on the site.

## API

Structured API summary generated from build-time component metadata.

- Exported symbols (1): ActionsCopy.
...
- `text`: The text to copy. Falls back to the default slot content. (type `string`; optional)
- `label`: Accessible label. (type `string`; optional)
- `disabled`: Whether the button is disabled. (type `boolean`; optional)
- `onCopy`: Emitted after a copy attempt, with the copied text. (type `((text: string) => void)`; optional)
