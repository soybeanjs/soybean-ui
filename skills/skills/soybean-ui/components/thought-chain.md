...
Usage examples for thought-chain are rendered on the site.
...
Interactive demos for thought-chain are rendered on the site.

## API

Structured API summary generated from build-time component metadata.

- Exported symbols (1): ThoughtChain.
...
- `items`: The reasoning steps to display. (type `ThoughtChainItem[]`; required)
- `defaultExpand`: Whether expandable steps are expanded by default. (type `boolean`; optional)
