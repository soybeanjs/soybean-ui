...
Usage examples for folder are rendered on the site.
...
Interactive demos for folder are rendered on the site.

## API

Structured API summary generated from build-time component metadata.

- Exported symbols (1): Folder.
...
- `name`: The folder name. (type `string`; required)
- `count`: Number of items to show as a badge. (type `number`; optional)
- `defaultOpen`: Whether the folder is expanded by default. (type `boolean`; optional)
