...
Usage examples for welcome are rendered on the site.
...
Interactive demos for welcome are rendered on the site.

## API

Structured API summary generated from build-time component metadata.

- Exported symbols (1): Welcome.
...
- `title`: Greeting title. (type `string`; optional)
- `description`: Description text. (type `string`; optional)
- `prompts`: Recommended prompts to show below the copy. (type `Prompt[]`; optional)
- `onSelectPrompt`: Emitted when a prompt is clicked. (type `((prompt: Prompt) => void)`; optional)
