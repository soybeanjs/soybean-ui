...
Usage examples for actions-feedback are rendered on the site.
...
Interactive demos for actions-feedback are rendered on the site.

## API

Structured API summary generated from build-time component metadata.

- Exported symbols (1): ActionsFeedback.
...
- `value`: The currently active feedback, if any. (type `FeedbackValue | null`; optional)
- `disabled`: Whether the buttons are disabled. (type `boolean`; optional)
- `onChange`: Emitted when feedback is given; `null` clears the active state. (type `((value: FeedbackValue | null) => void)`; optional)
