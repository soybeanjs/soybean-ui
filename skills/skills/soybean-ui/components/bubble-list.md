...
Usage examples for bubble-list are rendered on the site.
...
Interactive demos for bubble-list are rendered on the site.

## API

Structured API summary generated from build-time component metadata.

- Exported symbols (1): BubbleList.
...
- `items`: The message list to render. (type `ChatMessage[]`; required)
- `scrollable`: Whether the container has a constrained height and should be scrollable. When `false`, the list grows to fit its content. (type `boolean`; optional)
- `scrollThreshold`: Distance (px) from the bottom considered "at bottom". Defaults to `40`. (type `number`; optional)
- `showBackToBottom`: Whether to show the "back to bottom" button. Defaults to `true`. (type `boolean`; optional)
