...
1. Load the project skill at `.agents/skills/vean-develop/` — start with `SKILL.md`, then `layers.md` (Aria section) and `surfaces.md` (Playground, Docs, Testing sections when the change touches delivery surfaces).
...
- Stable, data-driven composite structure should prefer aria `*Compact` implementations instead of pushing iteration and default content up into the UI layer. Compact does not admit a new family; new families must pass [Aria admission](../../../../.agents/skills/vean-develop/layers.md#aria-admission). Existing anatomy-shell violations: [docs/aria-admission-remediation.md](../../../../docs/aria-admission-remediation.md).
