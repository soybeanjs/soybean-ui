import type { PageEllipsis, Pages } from './types';

const ELLIPSIS: PageEllipsis['type'] = 'ellipsis';
...
export function getRange(currentPage: number, pageCount: number, siblingCount: number, showEdges: boolean) {
  ...
  if (showEdges) {
    /**
     * `2 * siblingCount + 5` explanation: 2 * siblingCount for left/right siblings 5 for 2x left/right ellipsis, 2x
     * first/last page + 1x current page
     *
