...
- **Tech-stack alignment:** **strong** — sbean templates wire `@soybeanjs/cva ^0.0.9`, `@soybeanjs/headless`, `@soybeanjs/shadcn-theme`, `@soybeanjs/unocss-shadcn`, `unocss`; registry.json's 90+ components depend on `@soybeanjs/headless`; the `Icon` component depends on `@iconify/vue` (component-based icons, not class-based).
- **Architectural inconsistency:** the registry-item `uno` field is **opaque** `{config: object{}}` while the project-config `uno` (in `sbean.json`) is **structured** (`base/primary/feedback/size/radius`). This split-identity is the single biggest schema smell.
...
| ---------- | :---: | :--------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init`     |   ✓   |     ✓      | shadcn `init` offers base/style/icon/font/CSS-vars/RTL/pointer; sbean `init` wires `uno.config.ts` + `@soybeanjs/unocss-shadcn`                                                             |
...
| `info`     |   ✓   |     ✓      | sbean `info` prints `uno.base/primary/feedback/size/radius` ([info.ts:77-82](../src/commands/info.ts#L77-L82))                                                                              |
...
| `scan`     |   ✓   |     ✗      | sbean-only — detects `unocss`/`@unocss/core`/`@soybeanjs/unocss-shadcn` ([scan.ts:31-35](../src/commands/scan.ts#L31-L35))                                                                  |
...
| `cssVars`                   | `{light, dark}` (2 scopes)                                                                                                                    | `{theme, light, dark}` (3 scopes)                           | **GAP** (theme scope) → D3: stays lean in sbean; theme scopes owned by `@soybeanjs/shadcn-theme`   |
...
| **Framework detection** | `scan` recognizes `unocss`/`@unocss/core`/`@soybeanjs/unocss-shadcn` + `uno.config.{ts,js,mjs}` | `frameworks.ts` detects Nuxt/Vite/Laravel/Astro                                     | sbean is UnoCSS-focused; **D10** adds Nuxt awareness                                                                                                                                                |
...
- `scan.ts` recognizes `unocss`, `@unocss/core`, `@soybeanjs/unocss-shadcn` ([scan.ts:31-35](../src/commands/scan.ts#L31-L35)).
- `templates.ts` generates `uno.config.ts` with `presetSbean` from `@soybeanjs/unocss-shadcn` ([templates.ts:45](../src/templates/templates.ts#L45)).
...
- `registry/config.ts` imports from `@soybeanjs/shadcn-theme` ([config.ts:7](../src/registry/config.ts#L7)) — the theme package (built on `cva`) owns tokens, consistent with **D3** (sbean = delivery system; theme pkg = token owner).
...
Replace opaque `config: v.optional(v.object({}))` with a typed `sbeanBaseConfigSchema`: `uno` (base/primary/feedback/size/radius), `aliases` (ui/theme/styles/components/composables), `themePackage` (`@soybeanjs/shadcn-theme`), `resolver`, `iconLibrary`, `rtl`, `pointer`. Base items carry `.deepPartial()` of this.
