...
- **Tech-stack alignment:** **strong** — sbean templates wire `@soybeanjs/cva ^0.0.9`, `@soybeanjs/headless`, `@soybeanjs/theme`, `@soybeanjs/ui-unocss`, `unocss`; registry.json's 90+ components depend on `@soybeanjs/headless`; the `Icon` component depends on `@iconify/vue` (component-based icons, not class-based).
...
| ---------- | :---: | :--------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init`     |   ✓   |     ✓      | shadcn `init` offers base/style/icon/font/CSS-vars/RTL/pointer; sbean `init` wires `uno.config.ts` + `@soybeanjs/ui-unocss`                                                                 |
...
| `scan`     |   ✓   |     ✗      | sbean-only — detects `unocss`/`@unocss/core`/`@soybeanjs/ui-unocss` ([scan.ts:31-35](../src/commands/scan.ts#L31-L35))                                                                      |
...
| **Framework detection** | `scan` recognizes `unocss`/`@unocss/core`/`@soybeanjs/ui-unocss` + `uno.config.{ts,js,mjs}` | `frameworks.ts` detects Nuxt/Vite/Laravel/Astro                                     | sbean is UnoCSS-focused; **D10** adds Nuxt awareness                                                                                                                                                |
...
- `scan.ts` recognizes `unocss`, `@unocss/core`, `@soybeanjs/ui-unocss` ([scan.ts:31-35](../src/commands/scan.ts#L31-L35)).
- `templates.ts` generates `uno.config.ts` with `presetSbean` from `@soybeanjs/ui-unocss` ([templates.ts:45](../src/templates/templates.ts#L45)).
