...
import type { NavMenuCompactProps, NavMenuUi } from '@soybeanjs/headless/nav-menu';
import type {
...
import type { TreeMenuCompactProps, TreeMenuUi } from '@soybeanjs/headless/tree-menu';
 ...
 * single-pane shells (one nested `STreeMenu`, or one popup `SNavMenu`).
 *
 ...
 */
export type AppShellLogoPlacementResolved = 'sidebar' | 'header';

/**
 * Logo slot placement, `'auto'` follows the mode skeleton.
 */
 ...
 *
 * A superset of the `TreeMenu` / `SplitNav` option data, so one tree feeds the
 * nested sidebar, the split panes, and the top navigation menu alike.
 *
 * Not every renderer honors every field: `hidden` is applied by the shell for
 * all of them, while `selected` is only read by the top navigation menu
 * (`top` mode), where route-driven shells mark the active link.
 */
export interface AppShellMenuItem extends SplitNavBaseOptionData {
  /** Child options. */
  children?: AppShellMenuItem[];
  /**
   * Whether the option is the active link. Only the `SNavMenu` renderer
   * (`top` mode) reads it; the tree renderers derive selection from `modelValue`.
   */
  selected?: boolean;
}
...
export interface AppShellMenuProps {
  ...
  split?: Omit<
    ...
    | 'class'
  >;
  /** Forwarded to `SNavMenu` (`top` mode). */
  nav?: Omit<NavMenuCompactProps, 'items' | 'modelValue' | 'defaultValue' | 'class'>;
}
...
export interface AppShellMenuUi {
  ...
  /** Slot classes for `SNavMenu` (`top` mode). */
  nav?: Partial<NavMenuUi>;
}
...
export interface AppShellProps {
  ...
  modelValue?: string;
  /**
   * The active menu value when initially rendered.
   */
  defaultValue?: string;
  /**
   ...
   * whose menu has children opens a dropdown of those children. Provide it to
   * render a plain breadcrumb from your own data instead.
   */
 ...
 */
export interface AppShellLogoSlotProps {
  /**
   * Whether the shell is collapsed.
   */
  collapsed: boolean;
  /**
   * Region the logo is rendered in.
   */
...
export interface AppShellSlots {
  ...
  default?: () => any;
  /**
   * Custom content for the brand region.
   *
   * The shell renders no logo of its own: the app title and its description
   * belong to the injected content.
   */
  logo?: (props: AppShellLogoSlotProps) => any;
  /**
