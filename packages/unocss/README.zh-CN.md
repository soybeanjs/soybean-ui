...
SoybeanUI 的 UnoCSS 预设——`@soybeanjs/theme` token 与 UnoCSS 之间的唯一适配层。

## 📖 简介

`@soybeanjs/ui-uno` 把 [`@soybeanjs/theme`](../theme/README.md) 的声明式 token 契约转成 UnoCSS 的 theme 条目、preflight 与工具类。它是**唯一**的 UnoCSS 适配器:token 归属仍在主题包,而本预设消费的对象与运行时 `SConfigProvider` 用的是同一份选项,因此编译出的类名与注入的 CSS 不会分叉。

`presetUi()` 一次调用即返回完整推荐栈:

```
[presetWind3, presetAnimations, presetScrollbar, (presetWebFonts), ui-uno 主题预设]
```
...
主题选项(`base` / `primary` / `feedback` / `surfaceStyle` / `size` / `radius` / `spacing` / `overrides` / `format` / `darkSelector`…)会原样转发给 `@soybeanjs/theme`;`darkSelector` 经 `resolveWind3Dark()` 映射到 `presetWind3` 的 `dark` 配置。

### presetSbean

`npx sbean init` 之后,`uno.config.ts` 可以简化为单个预设——它读取 `sbean.json` 并据此推导主题:
...
- `sbean.json` 缺失或不可读时回退到默认主题(zinc / indigo / md)。
...
| `uiCSS`                               | `false`              | 是否包含 UI/主题 token preflight(按钮、输入框…);`presetSbean` 默认开启                          |
...
| :----------------- | :--------------------------------------------- |
| `presetUi`         | 完整推荐预设栈                                 |
| `presetSbean`      | 由 `sbean.json` 驱动的预设(包装 `presetUi`)    |
...
类型:`UiUnocssOptions`、`SbeanPresetOptions`、`PresetAnimationsOptions`、`PresetScrollbarOptions`。
