...
This scoped AGENTS file is a routing bridge for assistants editing `packages/ui-x/**`.

Before editing files here:

1. Load the project skill at `.agents/skills/soybean-ui-develop/` — start with `SKILL.md`, then `layers.md` (UI Layer section) and `surfaces.md`.
...
- Package: `@soybeanjs/ui-x` (peripheral package, single-package — domain logic + styles coexist in `src/`)
- Component prefix: `Sx`
