 ...
 *
 * Useful for parsing strings from data attributes, which are always strings, to the same type being used by the date
 * component.
 */
export function parseStringToDateValue(dateStr: string, referenceVal: DateValue): DateValue {
  const conditions: [boolean, DateValue][] = [
    [isZonedDateTime(referenceVal), parseZonedDateTime(dateStr)],
    [isCalendarDateTime(referenceVal), parseDateTime(dateStr)]
  ];

  const dateValue: DateValue = conditions.find(([condition]) => condition)?.[1] ?? parseDate(dateStr);

  if (dateValue.calendar !== referenceVal.calendar) {
    return toCalendar(dateValue, referenceVal.calendar);
  }
...
/**
 * Given a `DateValue` object, convert it to a native `Date` object. If a timezone is provided, the date will be
 * converted to that timezone. If no timezone is provided, the date will be converted to the local timezone.
 */
...
export function getDaysInMonth(date: Date | DateValue) {
  if (date instanceof Date) {
    ...
    /** By using zero as the day, we get the last day of the previous month, which is the month we originally passed in. */
...
/**
 * A helper function used throughout the various date builders to generate a default `DateValue` using the
 * `defaultValue`, `defaultPlaceholder`, and `granularity` props.
 *
 * It's important to match the `DateValue` type being used elsewhere in the builder, so they behave according to the
