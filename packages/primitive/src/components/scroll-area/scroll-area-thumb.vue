...
const props = defineProps<ScrollAreaThumbPropsWithPrimitive>();
...
watchOnce(sizes, () => {
  ...
  if (viewport.value) {
     ...
     * requestAnimationFrame loop that checks for changes to scroll position. That rAF loop triggers our thumb position
     * change when relevant to avoid scroll-linked effects. We cancel the loop when scroll ends.
