...
import type { AnyExceptLiteral, DateFormatter, DateStep, HourCycle, SegmentPart, SegmentValueObj } from '../date';
...
function dayPeriodSegmentAttrs(props: SegmentAttrProps) {
  ...
  const valueMin = 0;
  const valueMax = 12;
  let valueNow = segmentValues.hour || 0;
  if (valueNow > 12) {
    valueNow -= 12;
  }
...
export function useDateField(props: UseDateFieldProps) {
    ...
    if (part === 'day') {
      return dateRef
        .set({
           ...
           *
           * 1. If the month is filled, we need to ensure that the day snaps to the maximum value of that month.
           * 2. If the month is not filled, we default to the month with the maximum number of days (here just using
           *    January, 31 days), so that user can input any possible day.
           */
  ...
  function updateDayOrMonth(max: number, num: number, prev: number | null) {
    ...
    /**
     * If the user has left the segment, we want to reset the `prev` value so that we can start the segment over again
    ...
    if (prevValue === null) {
      /**
       * If the user types a 0 as the first number, we want to keep track of that so that when they type the next
      ...
      /**
       * If the last key was a 0, or if the first number is greater than the max start digit (0-3 in most cases), then
       * we want to move to the next segment, since it's not possible to continue typing a valid number in this
       * segment.
       */
      ...
      /**
       * If none of the above conditions are met, then we can just return the number as the segment value and continue
       * typing in this segment.
       */
      return { value: num, moveToNext };
    }

    /**
     * If the number of digits is 2, or if the total with the existing digit and the pressed digit is greater than the
     * maximum value for this month, then we will reset the segment as if the user had pressed the backspace key and
     * then typed the number.
     */
    const digits = prevValue.toString().length;
    const total = Number.parseInt(prevValue.toString() + num.toString(), 10);
    /**
     * If the number of digits is 2, or if the total with the existing digit and the pressed digit is greater than the
     * maximum value for this month, then we will reset the segment as if the user had pressed the backspace key and
     * then typed the number.
    ...
    if (digits === 2 || total > max) {
      /**
       * As we're doing elsewhere, we're checking if the number is greater than the max start digit (0-3 in most
       * months), and if so, we're going to move to the next segment.
       */
  ...
  function updateYear(num: number, prev: number | null) {
    ...
    if (prevValue === null) return { value: num === 0 ? 1 : num, moveToNext };

    const str = prevValue.toString() + num.toString();
