...
**Package:** `packages/cli/` → publishes as **`sbean`** (bin `sbean`), the _consumer-facing_ copy-paste CLI.
...
- Config lives in `sbean.json` (valibot-validated via `src/registry/config.ts`).
- Components come from the registry built out of `packages/ui/src` + `packages/headless`.
...
- URLs are env-overridable in `src/registry/constants.ts`: `REGISTRY_URL` (default `https://ui.soybeanjs.cn/r`), `DOCS_URL`, and the built-in namespaces `@soybean` / `@sbean`. Tests that need a registry use the local server helper, not the network.
- `sbean scan` generates `registry.json` from the source tree, extracting npm `dependencies`, cross-component `registryDependencies`, and preserving descriptions/categories from an existing file. `UI_SOURCE_PATH` (`packages/ui/src`) is hard-coded — a component directory move breaks the scan silently, so check it when you relocate files.
...
- **NO second name/type catalog** that duplicates the registry schema. Presets (`base` / `primary` / `size` / `radius` / `font` families in `src/registry/config.ts`) are pulled straight from `@soybeanjs/theme` so a family added to the engine reaches the CLI without a second edit — preserve that.
