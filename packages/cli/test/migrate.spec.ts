...
function options(overrides: Partial<RebrandMigrateOptions> = {}): RebrandMigrateOptions {
  return { cwd: root, write: false, runtimeContract: false, cli: false, force: false, ...overrides };
}
...
describe('migrate rules', () => {
  ...
  it('applies --repo-slug and --new-cdn on their own', () => {
    ...
    expect(applyRules(repo, buildRules({ repoSlug: 'soybeanjs/vean' })).text).toBe(
      'https://github.com/soybeanjs/vean/tree/main/docs'
    );
  ...
  it('normalizes hostnames and repository slugs', () => {
    ...
    expect(normalizeRepoSlug('https://github.com/soybeanjs/vean')).toBe('soybeanjs/vean');
...
describe('migrate preflight', () => {
  ...
  it('accepts legacy projects, explicit host rewrites, and --force', () => {
    ...
    expect(shouldMigrate(hostnameOnly, { force: false, repoSlug: 'soybeanjs/vean' })).toBe(true);
...
describe('runRebrandMigration', () => {
  ...
  it('renames the CLI config file when --cli is used', async () => {
    ...
    expect(dryRun.configRename).toEqual({ from: 'sbean.json', to: 'vean.json', status: 'would-rename' });
  ...
  it('keeps an existing vean.json instead of overwriting it', async () => {
    ...
    expect(report.configRename).toEqual({ from: 'sbean.json', to: 'vean.json', status: 'target-exists' });
...
describe('rebrandManualSteps', () => {
  ...
  it('only lists the framework steps the run actually triggered', async () => {
    await createProject({
      'src/main.ts': "import { SButton } from '@soybeanjs/ui';\n",
      'uno.config.ts': "import { presetUiUnocss } from '@soybeanjs/ui-uno';\n"
    });
