...
- **Command surface:** vean (14) **exceeds** shadcn-vue (11) — vean adds `template`, `registry`, `preset`, `scan`; vean is missing only `migrate`.
...
**Net:** vean **+4 unique commands**, **−1 missing command** (`migrate`). Per D4, `migrate` is **deferred** — added only when a real breaking Vean API change demands it.
