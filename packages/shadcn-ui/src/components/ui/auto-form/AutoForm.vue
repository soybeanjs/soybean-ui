...
const shapes = computed(() => {
  const val = {} as Record<keyof T, Shape>;
...
const fields = computed(() => {
  const val = {} as Record<keyof z.infer<T>, { shape: Shape; fieldName: string; config: ConfigItem }>;
...
const formComponentProps = computed(() => {
  if (props.form) {
    return {
      onSubmit: props.form.handleSubmit(val => emit('submit', val))
    };
  }
  ...
  return {
    ...
    onSubmit: (val: GenericObject) => emit('submit', val)
  };
});
</script>
