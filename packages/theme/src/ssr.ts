...
import { THEME_CSS_STORAGE_KEY, THEME_STORAGE_KEY } from './storage';
import type { DarkSelectorValue } from './types';

/**
 * the id of the `<style>` element the inline init script injects when
 * `injectCss` is enabled.
 *
 * The style carries `!important` declarations so it wins over the server
 * rendered default theme (which lives later in the document). `SConfigProvider`
 * removes it once the reactive theme style is applied on the client, so the
 * `!important` declarations cannot block runtime theme changes.
 */
export const THEME_INIT_STYLE_ID = '__SOYBEAN_THEME_INIT';
...
export interface ThemeInitScriptOptions {
  ...
  storageKey?: string;
  /**
   * whether to inject the persisted theme CSS snapshot before first paint.
   *
   * The derived color / radius / size tokens cannot be recomputed in an inline
   * script, so the CSS generated by the app is persisted separately and read
   * back here. It is injected with `!important` so it overrides the default
   * theme rendered by SSR (which sits later in the document). Disabled by
   * default to keep the script minimal for consumers that do not need it.
   *
   * @defaultValue false
   */
  injectCss?: boolean;
  /**
   * the localStorage key the persisted theme CSS snapshot is read from.
   *
   * Must match the key used by `setStoredThemeCss` (default
   * `THEME_CSS_STORAGE_KEY`). Only used when `injectCss` is enabled.
   *
   * @defaultValue THEME_CSS_STORAGE_KEY
   */
  cssStorageKey?: string;
  /**
 ...
 * - optionally injects the persisted theme CSS snapshot (`injectCss`) so the
 *   derived color / radius / size tokens are correct before first paint
 *
...
export function createThemeInitScript(options: ThemeInitScriptOptions = {}): string {
  const {
    ...
    injectCss = false,
    cssStorageKey = THEME_CSS_STORAGE_KEY,
    darkSelector = 'class',
    setDataTheme = true
  } = options;
  ...
  // the derived CSS cannot be recomputed inline, so it is read back from the
  // persisted snapshot and marked `!important` to win over the SSR-rendered
  // default theme, which appears later in the document.
  if (injectCss) {
    statements.push(
      `    var css = localStorage.getItem(${JSON.stringify(cssStorageKey)});`,
      '    if (css) {',
      "      var initStyle = document.createElement('style');",
      `      initStyle.id = ${JSON.stringify(THEME_INIT_STYLE_ID)};`,
      "      initStyle.textContent = css.replace(/(--[\\w-]+:\\s*[^;{}]+);/g, '$1 !important;');",
