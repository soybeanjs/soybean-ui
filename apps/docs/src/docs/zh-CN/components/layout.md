...
- **两种布局模式** — `SLayout`（现代侧边栏 + 主内容流）和 `SLayoutClassic`（管理后台经典布局，支持滚动容器、固定头部/底部、方向切换）。
...
- **经典方向** — `LayoutClassic` 支持 horizontal（侧边栏在内容旁）与 vertical（侧边栏在 header 下方）两种方向。
- **经典滚动行为** — `scrollBehavior="content"` 仅滚动内容区域；`scrollBehavior="wrapper"` 滚动整个 main 容器。
...
| 固定头部/底部       | `LayoutClassic` 的 `fixedTop` / `fixedFooter` + 自动占位元素                       | 需要手动 sticky CSS                                     | 需要手动 sticky CSS                                                 |
| 方向                | `LayoutClassic` `orientation="horizontal" \| "vertical"`                           | —                                                       | —                                                                   |
| 滚动行为            | `LayoutClassic` 的 `wrapper` / `content`                                           | —                                                       | —                                                                   |
...
5. **`LayoutClassic` 占位元素** — 启用 `fixedTop` 或 `fixedFooter` 时，`LayoutClassicPlaceholder` 渲染空的占位 div（`data-soybean-layout-{header|tab|footer}-placeholder`），防止内容滑入固定区域下方。
6. **`scrollId` 用于滚动恢复** — `LayoutClassic` 在滚动元素（wrapper 或 content，取决于 `scrollBehavior`）上生成稳定的 `soybean-layout-scroll-{id}`。传入 `scrollId` 可使其在 SSR/CSR 间确定一致。

## 常见问题

### 应该使用 `SLayout` 还是 `SLayoutClassic`？

`SLayout` 适用于现代应用外壳——侧边栏位于单一滚动内容区旁。`SLayoutClassic` 适用于需要固定头部/底部、方向切换或容器级滚动 + 占位间距的管理后台。
...
`LayoutClassic` 接受 `baseZIndex`（默认 `50`）。sidebar、header、tab、footer 的 z-index 均由此基础值派生，确保堆叠可预测。派生值通过 `--soybean-layout-{sidebar|header|tab|footer}-z-index` CSS 变量暴露。
