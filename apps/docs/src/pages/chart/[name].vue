...
const route = useRoute('/chart/[name]');
const { t } = useI18n();

const name = computed(() => route.params.name);

const path = computed(() => `chart/components/${name.value}`);
...
const currentGroup = computed(() => {
  return chartMenuData.find(group => group.items.some(item => kebabCase(item) === name.value));
});
        ...
        <div class="space-y-5">
          ...
          <div class="space-y-4">
            <div class="flex flex-wrap items-center gap-2.5">
              ...
              <div class="code-btn-outline">@soybeanjs/chart</div>
