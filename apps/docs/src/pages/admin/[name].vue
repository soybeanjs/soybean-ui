...
const currentGroup = computed(() => {
  return adminMenuData.find(group => group.items.some(item => kebabCase(item) === name.value));
});
  ...
  return (
    currentGroup.value?.items
      ...
      .map(item => ({
        ...
        to: localizedTo(`/admin/${kebabCase(item)}`)
      })) ?? []
  );
});
        ...
        <div class="space-y-5">
          ...
          <div class="space-y-4">
            <div class="flex flex-wrap items-center gap-2.5">
              ...
              <div class="code-btn-outline">@soybeanjs/admin</div>
