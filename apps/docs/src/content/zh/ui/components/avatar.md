...
head:
  title: 头像
  description: 用于展示用户头像的图片组件，支持加载失败时的回退内容。SAvatar 组合 AvatarRoot/AvatarImage/AvatarFallback 这一系列 headless 基础组件（零样式）与 avatarVariants 样式配方（3 个槽：root/image/fallback，xs–2xl 共 6 种尺寸）。
---
