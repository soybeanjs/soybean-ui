...
- `TimeRangeFieldCompact`（headless）— `TimeRangeFieldRoot` + 两组各段一个 `TimeRangeFieldInput` 的数据驱动组合，含 `leading`/`separator`/`trailing` 插槽；无样式用法从 `@vean/aria/time-range-field` 导入
