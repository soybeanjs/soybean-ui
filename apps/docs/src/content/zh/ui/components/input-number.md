...
Vean 将数字输入框拆分为负责状态、数字解析/格式化、边界逻辑与表单代理的 headless 层（`@vean/aria/input-number`），以及负责变体与 UnoCSS 样式的 styled 层（`@vean/ui`）。headless `InputNumberCompact` 组合 `InputNumberRoot` / `InputNumberControl` / `InputNumberIncrement` / `InputNumberDecrement` / `InputNumberClear` 并暴露 `increment` / `decrement` / `clear` 插槽。这与 headless/styled 分离一致，区别于 Ant Design、Element Plus、Mantine 等单包方案。

| 能力                     | Vean | Ant Design `InputNumber` | Element Plus `input-number` | Mantine `NumberInput` |
| :----------------------- | :--: | :----------------------: | :-------------------------: | :-------------------: |
| headless/styled 分离     |  ✅  |            —             |              —              |           —           |
| 受控/非受控              |  ✅  |            ✅            |             ✅              |          ✅           |
| step / min / max / 精度  |  ✅  |            ✅            |             ✅              |          ✅           |
