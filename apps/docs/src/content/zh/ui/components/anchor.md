...
> `SAnchor` 现在会把递归的锚点项渲染委托给 headless `AnchorCompact`。`SAnchor` 与 `@vean/aria/anchor` 共享同一组 6 个 `ui` 槽位。
...
- **Headless 组合** — `AnchorRoot` / `AnchorLink` / `AnchorCompact` / `AnchorItemCompact` 均可从 `@vean/aria/anchor` 导出，用于完全自定义样式。

## 用法

<UsageCode component="anchor" />

> `SAnchor` 现在将递归项渲染委托给 headless `AnchorCompact`。无样式的数据驱动用法可导入 `@vean/aria/anchor` 的 `AnchorCompact`。
...
| 能力                 | Vean                                                          | Ant Design `Anchor`             | Element Plus `Anchor` |
| :------------------- | :------------------------------------------------------------ | :------------------------------ | :-------------------- |
| headless/styled 分离 | ✅ `@vean/aria/anchor` + `scv()`                              | ❌ 单包                         | ❌ 单包               |
