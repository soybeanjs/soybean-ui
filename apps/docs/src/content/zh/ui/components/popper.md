...
head:
  title: Popper 定位浮层
  description: 底层浮动原语：将内容锚定到触发元素，并提供完整的触发、开合与 dismiss 行为。SPopper 组合 headless PopperRoot/PopperTrigger/PopperAnchor/PopperPositioner/PopperPopup/PopperArrow/PopperSub 基础组件家族（构建于 Floating UI 之上）与 popperVariants 样式配方（6 个槽、3 种尺寸）。
---
