...
head:
  title: 空状态
  description: 用于突出「内容缺失」并引导用户采取下一步操作的轻量级空状态组件。SEmpty 组合 EmptyRoot/EmptyHeader/EmptyMedia/EmptyTitle/EmptyDescription/EmptyContent 这一系列 headless 基础组件（零样式）与 emptyVariants 样式配方（6 个槽、6 种尺寸）。
---
