...
head:
  title: PalettePicker
  description: SPalettePicker 是一个基于单一基础色生成完整色板的颜色选择器。其 modelValue 是一个 ColorValue——可以是 Tailwind 色板键 + 档位（如 indigo.500）、简单色板键（black / white / transparent …），或原始自定义颜色（hsl(...) / oklch(...)）。
---
...
`SPalettePicker` 是一个基于单一基础色生成完整色板的颜色选择器。其 `modelValue` 是一个 [`ColorValue`](/overview/theming)——可以是 Tailwind 色板键 + 档位（如 `indigo.500`）、简单色板键（`black` / `white` / `transparent` …），或原始自定义颜色（`hsl(...)` / `oklch(...)`）。
