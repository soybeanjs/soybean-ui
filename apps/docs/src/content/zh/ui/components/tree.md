...
head:
  title: 树
  description: 用于展示树形层级数据、并支持选择与展开/折叠的组件。STree 将 headless 层 TreeRoot 一族基础原语（TreeRoot/TreeItem/TreeVirtualizerRoot/TreeVirtualizerItem，零样式）与 TreeRoot 上下文（受控/非受控选中与展开、单选/多选、级联选择、Roving Focus 键盘导航）组合；STreeVirtualizer 在其上叠加虚拟滚动，可平滑承载 1000+ 节点。节点内容完全由 item 插槽自定义，节点图标、勾选框、缩进与连接线样式均由使用方注入。
---
