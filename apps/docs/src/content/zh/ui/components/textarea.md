...
head:
  title: 文本域
  description: 用于输入多行文本的组件，支持自动高度、字符计数与可清除功能。适用于描述、评论、留言等跨行内容。单行输入请使用 SInput。
---
...
Vean 将文本域拆分为负责状态、自动高度测量与表单代理的 headless 层（`@vean/aria/textarea`），以及负责变体与 UnoCSS 样式的 styled 层（`@vean/ui`）。headless `TextareaCompact` 组合 `TextareaRoot` / `TextareaControl` / `TextareaClear` / `TextareaCounter` 并暴露 `clear` / `counter` / `footer` 插槽。这与 Radix 的 headless/styled 分离一致，区别于 Ant Design、Element Plus、Mantine 等单包方案。

| 能力                   | Vean | Ant Design `Input.TextArea` | Element Plus `Input` | Mantine `Textarea` |
| :--------------------- | :--: | :-------------------------: | :------------------: | :----------------: |
| headless/styled 分离   |  ✅  |              —              |          —           |         —          |
| 受控/非受控            |  ✅  |             ✅              |          ✅          |         ✅         |
| autosize（min/max 行） |  ✅  |             ✅              |          ✅          |         ✅         |
