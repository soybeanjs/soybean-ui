...
head:
  title: 单选框组
  description: 一组可勾选的单选框按钮，同一时刻最多只有一个按钮处于选中态。以数据驱动分组与带 icon/description 内容的卡片变体提供，两者均基于 roving-focus 键盘导航。适用于从小型互斥集合中单选；允许多选时请用 SCheckboxGroup；选项集合过大无法一眼扫尽时，优先用 SSelect。
---
