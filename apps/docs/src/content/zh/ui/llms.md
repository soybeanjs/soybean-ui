...
head:
  title: LLMs.txt
  description: 介绍如何在 Cursor、Windsurf、GitHub Copilot、ChatGPT、Claude 等 AI 工具中使用 Vean 生成的 LLM 友好文档。
---

# LLMs.txt

> 介绍如何在 Cursor、Windsurf、GitHub Copilot、ChatGPT、Claude 等 AI 工具中使用 Vean 生成的 LLM 友好文档。

## 什么是 LLMs.txt？

LLMs.txt 是一种面向大语言模型的结构化文档格式。Vean 会在文档构建阶段生成专门的 LLM 友好文件，让 AI 工具比直接读取渲染后的站点页面更稳定地获取组件文档、API 信息和使用建议。

这些生成文件专门针对 AI 消费场景做了处理，目前以英文文档作为导出来源。

## 可用路由

Vean 文档构建后会暴露以下面向 LLM 的路由：
...
你可以在 Cursor 的提问中直接引用 Vean 文档路由，或者通过文档上下文能力把这些路由加进去。
