...
head:
  title: Playground
  description: 'Run the component examples in the browser and switch between the UI, UI-X and Chart libraries.'
---
...
- **Library** — the select in the card header switches between `Headless`, `UI`, `UI-X`, `Admin` and `Chart`. Examples currently exist for `UI`, `UI-X` and `Chart`; the remaining libraries show an empty state until their examples land.
