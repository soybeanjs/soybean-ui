...
import type { FocusOutsideEvent, PointerDownOutsideEvent } from '@soybeanjs/headless/types';
import type { EpOpenChangeReason, EpRootContext } from './types';

interface UsePopperDismissOptions {
  layerElement: ShallowRef<HTMLElement | undefined>;
  context: EpRootContext;
  /**
...
export function usePopperDismiss(options: UsePopperDismissOptions) {
  ...
  let dismissReason: Extract<EpOpenChangeReason, 'dismiss-outside' | 'dismiss-escape'> = 'dismiss-outside';
