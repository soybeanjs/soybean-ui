...
| D1-05 | Context reactivity               | Context values are `ComputedRef` or `ShallowRef`; prop-derived fields use `toContext` or `ToContext`                                                                                                                                                                                                                                                                                                                                                              | No bare value assignments in `context.ts`; no raw non-reactive values enter context                                                                 |
...
| D1-17 | Exports and generated files      | `packages/headless/src/index.ts` and `packages/ui/src/index.ts` are updated; `pnpm sui gen catalog headless` and `pnpm sui gen catalog ui` have run; no hand-edited generated files                                                                                                                                                                                                                                                                               | `git diff` contains no hand edits to `constants/components.ts` or `namespaced/index.ts`                                                             |
...
| D3-14 | Docs match implementation    | API data generated by `pnpm sui gen api` matches actual props/emits; doc examples are runnable                                                                                                                                                                     | `pnpm sui gen api` re-run diff         |
...
**Scope:** `apps/docs/src/content/{en|zh}/ui/components/{component}.md`, `apps/docs/src/examples/ui/{component}/`, `apps/docs/src/constants/menus.ts`, `apps/docs/src/generated/{api,changelog}/`.
...
| D6-05 | Playground coverage             | Playground examples cover major public capabilities (`color`/`size`/`disabled`/`ui` etc.); no duplicate scenes                                                                                                                                                | `apps/docs/src/examples/ui/{component}/` file list review |
| D6-06 | Playground naming               | Filenames `NN-name.vue`; `name` accurately describes the capability; drives i18n title keys                                                                                                                                                                   | Review                                                    |
| D6-07 | API data generation             | `pnpm sui gen api` has run; non-English locales ran `pnpm sui gen api --translate --locale <locale>`; no hand-edited generated data                                                                                                                           | git diff contains no hand edits to `generated/api/`       |
| D6-08 | Changelog data                  | After public API/changelog mapping changes, ran `pnpm sui gen changelog` and `pnpm sui gen changelog --translate --locale <locale>`                                                                                                                           | Same as above                                             |
...
| D7-09 | SSR compatibility                   | Components do not throw under SSR (ubean SSG); no direct `window`/`document` access; DOM only inside `onMounted`                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | SSR build passes                                          |
...
- [ ] Generation commands: `pnpm sui gen catalog headless` / `pnpm sui gen catalog ui` / `pnpm sui gen api` / `pnpm sui gen changelog` (and `--locale`) run as needed
...
- **SSR verification:** `pnpm build:docs` (ubean SSG build) + `pnpm --filter @soybeanjs/ui-docs preview`
...
# Generation
pnpm sui gen catalog headless       # regenerate headless constants + namespaced
pnpm sui gen catalog ui             # regenerate ui constants
pnpm sui gen api            # regenerate API data
pnpm sui gen api --translate --locale <locale>
pnpm sui gen changelog      # regenerate changelog data
pnpm sui gen changelog --translate --locale <locale>
