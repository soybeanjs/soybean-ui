...
| D1-19 | Headless admission               | New families pass the [deletion test](layers.md#deletion-test). Anatomy shells are not used as templates. Decorative Header/Footer slots are not promoted to a public primitive contract. See [layers.md -> Headless admission](layers.md#headless-admission). Shipped shells listed as 冻结 in [docs/headless-admission-remediation.md](../../../docs/headless-admission-remediation.md) are not Blockers. | New family: admitted, UI-only, or composed. Existing freeze-listed shells: mark — until CMP-6, do not copy                             |
