...
| `shared.ts`  | `mergeVariants(variants, ...uis)` — merges tv() output with UI context classes; `mergeUi(target, ...sources)` — merges class maps          |
...
- **mergeVariants**: Bridges `tv()` slot output with class tokens from headless `useUiContext`. Used in multi-slot components.
...
- Slot-based wrappers should prefer `mergeVariants`, while single-class wrappers should use `cn()` directly with `props.class`.
