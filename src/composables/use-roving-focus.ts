...
type RovingFocusGroupContextParams = PropsToContext<RovingFocusGroupProps> & EmitsToHookProps<RovingFocusGroupEmits>;

type RovingFocusItemOptions = Partial<PropsToContext<RovingFocusItemProps>>;

const ON_ENTRY_FOCUS = 'rovingFocusGroup.onEntryFocus';
const ROVING_FOCUS_EVENT_OPTIONS = { bubbles: false, cancelable: true };

export function useRovingFocus(name: string) {
  const contextName = toPascalCase(`${name}RovingFocusGroup`);

  const { provideCollectionContext, useCollectionContext, useCollectionItem } = useCollection(contextName);
