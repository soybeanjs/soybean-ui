...
> 规范约束：组件开发 skill（[.agents/skills/vean-develop/](../.agents/skills/vean-develop/SKILL.md)），尤其是 [layers.md Aria admission（R1–R8）](../.agents/skills/vean-develop/layers.md#aria-admission)
...
准入依据 [layers.md R1–R8](../.agents/skills/vean-develop/layers.md#aria-admission)。aria 零样式（连 `hidden` / `sr-only` 都不允许），UI 层不写任何 ARIA/键盘语义。
