...
> 组件规范:[`.agents/skills/vean-develop/`](../.agents/skills/vean-develop/SKILL.md)。
...
- popup transform 改为 CSS 变量驱动;UI 层 `styles/sheet.ts` 的 popup 槽声明 `transform: translateY(calc(var(--vean-drawer-snap-point-offset) + var(--vean-drawer-swipe-movement-y)))`(变量名沿用本仓 `--vean-*` 命名空间)。
