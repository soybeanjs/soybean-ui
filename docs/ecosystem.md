...
> 本文记录 SoybeanUI 周边 UI 组件生态（历史上的 ui-x / admin / chart，及未来 ui-pro / ui-lowcode 等）的依赖架构、文档展示、分发与扩展契约。
> 决策来源：`/grilling` 会话（3 轮）+ `CONTEXT.md` 术语表 + `docs/adr/0001-peripheral-package-layering.md`。
> 生成日期：2026-08-13。

> **状态更新（2026-09）：`@soybeanjs/ui-x` 已整包移除，仓库内当前无任何外围包。** AI/chat 组件改为按标准 headless/ui 两层契约在核心库内重新实现（统一 `S` 前缀），规划见 [`docs/ui-ai-roadmap.md`](./ui-ai-roadmap.md)；ADR-0001 对 AI 域失效（标记 superseded）。sbean 的多命名空间 registry 机制作为通用能力保留（当前 registry 仅发布 `ui` 命名空间）。下文涉及 ui-x 的内容均为历史规划记录。
>
> **状态更新（v0.40.0）：`@soybeanjs/admin` 与 `@soybeanjs/chart` 已移除。** 二者不再作为独立包发布；图表改为在文档站直接基于 [TanStack Charts](https://tanstack.com/charts) 展示 shadcn 风格示例（见 `apps/docs/src/examples/chart/` 与文档内的 `components/chart/` 主题壳），不进入核心库。下文涉及 admin / chart 的分层、分发、文档命名空间等内容保留作为历史规划记录。
...
| 包                      | 领域                                        | 当前状态                                                                                               |
| ----------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `@soybeanjs/ui-x`       | AI 组件（Bubble/Sender/ThoughtChain…）      | **已移除（2026-09）**：AI 组件改为核心库 headless/ui 内实现，见 [ui-ai-roadmap.md](./ui-ai-roadmap.md) |
...
Layer 4 · 外围包（单包自治：领域逻辑 + 样式同居）
   （当前无外围包：ui-x 于 2026-09 移除、admin / chart 于 v0.40.0 移除；
    AI 组件回归核心库，图表改为文档站 TanStack Charts 示例；
    未来外围包仍按本层契约接入）
        ▼
Layer 3 · 核心样式层
                @soybeanjs/ui
...
| `@soybeanjs/ui-x`     | —   | 已移除（2026-09）；AI 组件回归核心库               | —             | —                |
...
### 3.2 拆解 `headless-x`（历史记录，最终归宿已被新决策取代）

> 本节记录 ADR-0001 当时的执行过程。2026-09 起 ui-x 整包移除，这些 composables/types 随 AI 组件一起重新进入核心 headless/ui，新归属与命名见 [ui-ai-roadmap.md](./ui-ai-roadmap.md)（`useStream` / `useChat` 等）。

原外围 AI 线曾存在 `@soybeanjs/headless-x`（仅 composables + types，无 SFC），ADR-0001 执行时被拆解：内容一度迁入外围单包（use-chat / use-send / use-think / use-typing / use-x-stream / use-bubble-list-scroll / use-conversations / use-sender / use-thought-chain 等），最终于 2026-09 随该包移除、按核心库两层契约重新实现。

### 3.3 命名（历史记录）

- 外围 AI 样式包曾定名 `@soybeanjs/ui-x`（`-x` 作为"AI/扩展"品牌），该包已于 2026-09 移除；核心库内 AI 组件统一使用 `S` 前缀。
...
| `@soybeanjs/ui-x`       | ~~`Sx`~~（已移除） | ~~`SxBubble` `SxSender`~~；核心库重实现后为 `S` 前缀（如 `SThread` `SSender`） | 历史：强领域词汇表 → 2 字母前缀 |
...
判据（写入 `CONTEXT.md`「组件前缀」）：**领域词汇表强度**。当组件名构成该领域专属词汇表（AI 的 Bubble/Sender/ThoughtChain 在普通 UI 词汇中不存在）→ 历史上外围包用 2 字母前缀；回归核心库后统一使用 `S` 前缀（见 ui-ai-roadmap.md）。未来若有新外围包，仍可按此判据选择独立前缀；`@soybeanjs/admin` 当年包名保留（表意"应用壳"而非"另一种 UI 风格"），不改为 `ui-admin`。
...
### 5.2 composables 子路径暴露（外围包通用模式，历史示例）

> 历史上外围单包通过 `./composables`、`./types` 子路径暴露领域逻辑以缓解"逻辑锁进样式包"。当前无外围包；核心库 AI composables 直接从 `@soybeanjs/headless` 公共入口导出，无需子路径（见 ui-ai-roadmap.md）。未来若新增外围包，可沿用此模式：

```jsonc
{
  ".": "./src/index.ts",
  "./composables": "./src/composables/index.ts", // 可选
  "./types": "./src/types/index.ts", // 可选
...
当某个外围包 composable 被其它包需要时，按 §3.1「原子原语」判据评估是否上浮核心 `@soybeanjs/headless`；单域独享则留该外围包。

## 6. sbean registry（Q4）

### 6.1 命名空间 item

单一 `packages/sbean/registry.json`，顶层含 `packages` 元数据，items 为命名空间形式（当前实际只发布 `ui` 命名空间；下例保留历史规划中的多包形态以说明机制）：
...
```jsonc
{
  ...
  "items": [
    ...
    // 外围包条目形如 "<pkg>/<component>"，并附 "package": "<pkg>" 字段
  ]
}
```
...
- `sbean add <pkg>/<component>` —— 命名空间路径寻址（如未来的 `sbean add admin/app-layout`）。
- `sbean add button` —— 仍可用（核心 ui 裸名直查；非核心包重名时提示补命名空间）。
- `sbean list --package <pkg>` —— 按包列出。
- `sbean docs <item>` —— 输出对应命名空间文档链接（见 §7）。
...
| `/chart`      | （非包；TanStack Charts 文档示例） | `chartMenuData`  |
| `/admin` 等   | 未来外围包（历史规划）             | `<Pkg>MenuData`  |

每命名空间页面结构（多包时期验证过的模板，未来外围包沿用）：
...
- `[name].vue` —— 组件详情（按各包前缀渲染导入名）

### 7.2 顶部包切换器（Q6b=II）

`apps/docs/src/layouts/default.vue` 的 header 提供包切换入口，切换时跳转对应命名空间根并切换侧边栏 menu data（当前实际入口：UI / Chart / Headless / Playground；AI 组件进入核心库后不再有独立入口）。

`shouldShowSidebar` 由 `route.path.startsWith` 列表适配：`/overview`、`/components`、`/chart`、`/headless`。每加一个外围包追加一项 + 一个 `<Pkg>MenuData`。
...
apps/docs/src/generated/
  ...
  api-locales/
    ...
    headless/en.json  headless/zh-CN.json
  changelog/
    ui/...  headless/...
  changelog-locales/
    ui/...  headless/...
...
v0.40.0 playground 已并入 docs：每新增外围包都加入 `apps/docs/package.json` 的 `dependencies`（当前 docs 仅依赖核心 headless/ui；chart 示例直接依赖 `@tanstack/charts`）。
...
- `main` 是唯一长期分支。
- 历史上的外围包临时分支（ui-x / admin / `chart`）在合入或移除后均已归档/删除。
...
> 本节为 2026-08 多包时期的执行记录。§11.1/§11.2 的合并与拆解均已完成，其最终产物（外围 ui-x 包）也于 2026-09 移除，AI 组件回迁核心库（见 [ui-ai-roadmap.md](./ui-ai-roadmap.md)）；§11.3–11.5 中仍生效的是多命名空间机制本身。

### 11.1 三分支合并到 main（历史执行步骤）

1. 从 `main` 拉取最新基线。
2. 按依赖顺序合并：先 AI 线（拆解 headless-x，见 §11.2）→ 再 `chart`（新建包）→ 最后 `admin`（接入 `admin→chart` peerDep）。
3. 每步合并后跑 `pnpm typecheck` + `pnpm test` + `pnpm build` 验证。
4. 合并完成后归档/删除各临时分支，后续 feature branch 走 main。（其后 admin/chart 在 v0.40.0、AI 外围包在 2026-09 分别移除。）

### 11.2 拆解 headless-x（历史执行步骤，最终归宿见 §3.2）

1. 合并 AI 线时执行：`packages/headless-x/src/composables/*` 与 `types/*`、`test/specs/*` 迁入外围包。
2. 删除 `packages/headless-x/`。
3. 外围包 exports 增补 `./composables`、`./types`。
4. 评估 `use-x-stream` 是否上浮核心 headless（后续回迁时确定为核心库 AI composables，见 ui-ai-roadmap.md）。
...
3. 各外围包 examples 一律置于 `examples/<pkg>/` 子目录（AI 示例随包移除后，将随核心库 AI 组件重新落在 `examples/ui/`）。
...
3. 新增外围包时追加 `<pkg>/*` items（当前仅 `ui`；AI 组件进入核心库后以 `ui/*` 条目发布）。
...
- [ADR-0001 · 外围包分层模型](adr/0001-peripheral-package-layering.md) —— 记录"拆解 headless-x、外围包单包自治、核心 headless 唯一逻辑层"的决策与权衡（2026-09 起对 AI 域 superseded）。
- [ui-ai-roadmap.md](ui-ai-roadmap.md) —— AI/chat 组件回归核心 headless/ui 的重新实现规划（ADR A1 取代本方案的 AI 外围包决策）。
