...
function ComponentPreview(md: MarkdownRenderer) {
  md.core.ruler.after('inline', 'component-preview', state => {
    ...
    state.src = state.src.replace(regex, (_, bindingValue) => {
      ...
      const pathName =
        props.type === 'example' ? `../../components/examples/${props.name}` : `../../../components/demo/${props.name}`;
      insertComponentImport(
        props.type === 'example'
          ? `import ${props.name} from '${pathName}/index.vue'`
          : `import ${props.name} from '${pathName}/tailwind/index.vue'`
      );
      ...
      state.tokens[index].content =
        `<ComponentPreview name="${props.name}" type="${props.type || 'demo'}"  files="${encodeURIComponent(JSON.stringify(groupedFiles))}" ><${props.name} />`;
