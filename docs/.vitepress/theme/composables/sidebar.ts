...
export function useSidebar() {
  ...
  const hasAside = computed(() => {
    ...
    if (!isNullish(frontmatter.value.aside)) {
...
/**
 * a11y: cache the element that opened the Sidebar (the menu button) then focus that button again when Menu is closed
 * with Escape key.
 */
...
export function useSidebarControl(item: ComputedRef<DefaultTheme.SidebarItem>): SidebarControl {
  ...
  const collapsible = computed(() => {
    return !isNullish(item.value.collapsed);
  });
  ...
  const hasChildren = computed(() => {
    return Boolean(item.value.items && item.value.items.length);
  });
  ...
  watchPostEffect(() => {
    if (isActiveLink.value || _hasActiveLink.value) {
...
export function isActive(_currentPath: string, matchPath?: string, asRegex: boolean = false): boolean {
  if (matchPath === undefined) {
    return false;
  }

  const currentPath = normalize(`/${_currentPath}`);
...
/**
 * Get the `Sidebar` from sidebar option. This method will ensure to get correct sidebar config from
 * `MultiSideBarConfig` with various path combinations such as matching `guide/` and `/guide/`. If no matching config
 * was found, it will return empty array.
 */
export function getSidebar(_sidebar: DefaultTheme.Sidebar | undefined, _path: string): SidebarItem[] {
  if (Array.isArray(_sidebar)) return addBase(_sidebar);
  if (isNullish(_sidebar)) return [];
  ...
  const dir = Object.keys(_sidebar)
    ...
    .find(item => {
      ...
      return path.startsWith(ensureStartingSlash(item));
    });
...
export function hasActiveLink(path: string, items: SidebarItem | SidebarItem[]): boolean {
  ...
  return hasActiveLink(path, items.items);
}
