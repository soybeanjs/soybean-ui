...
> **2026-09 前提变更：** 本文写于「外围包单包自治」时期，标题中的 `@soybeanjs/table` 是当时设想的包名；当前仓库无外围包，文中包结构、lockstep、跨包依赖等设定已不适用。**市场调研结论与能力设计仍然有效**；表格内核选型与 [v0.50.0.md](../v0.50.0.md) 引擎更换联动，立项时须先按 [README §立项时必须回答的问题](./README.md#立项时必须回答的问题) 确定落地形态（并入核心 headless/ui、独立包、或 vean 源码配方）。
