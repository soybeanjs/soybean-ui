...
| [table.md](./table.md)                         | 高级数据网格 / ProTable          | 提案（调研已完成） | 表格内核已随 v0.50 落地为 `@tanstack/vue-table`；服务端数据源/查询/分页/编辑/列管理              |
...
1. **形态**：并入核心 headless/ui（须过 headless 准入 R1–R8）、独立 npm 包（须新立 ADR，不能默认复用 ADR-0001），还是仅以 vean 源码配方分发？
