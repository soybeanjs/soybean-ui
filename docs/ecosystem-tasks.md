...
> **2026-09 更新：本文中 `ui-x` 相关任务全部取消——`@soybeanjs/ui-x` 整包移除，AI/chat 组件改为在核心 headless/ui 内按组件开发规范重新实现，新任务体系以 [ui-ai-roadmap.md](./ui-ai-roadmap.md) 为准。** ui-x 行保留作为历史记录，不再执行。多命名空间 sbean / 生成链路等与具体包无关的机制性任务仍然有效。
>
> **v0.40.0 更新：本文中 `admin` / `chart` 相关任务（含 `admin → chart` peerDep 接线）全部取消，对应包已删除；图表改为文档站基于 TanStack Charts 的 shadcn 风格示例。** 这些行保留作为历史记录，不再执行。
