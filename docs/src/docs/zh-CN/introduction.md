...
- Headless 暴露多个分片组件（Root、Trigger、Content 等）
...
- 无 UiContext
- UI 层直接使用 `cn(variants(...), props.class)` 合并样式
...
- **cn() 工具**：Tailwind 感知的类名合并（`clsx` + `tailwind-merge`），无冲突组合
