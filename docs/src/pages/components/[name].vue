...
// 动态加载各组件的 index.md（支持多语言与默认）
const mdModules = import.meta.glob<{ default: Component }>('@components/**/docs/**/index.md');
const DocComp = shallowRef<any>(null);

async function loadDoc() {
  const candidates = [
    `../src/components/${name.value}/docs/${locale.value}/index.md`,
    `../src/components/${name.value}/docs/index.md`
  ];
