...
| C' Card 家族（原桶 C 附注）  |  1   | **改判 UI-only**，W2 执行（删 aria/card，SCard 组合 Collapsible）            | CMP-6.5           |
...
| ID     | 组件       | 证据                                                                                             | 违反   | 处置                                                                                                                  | 状态 |
| :----- | :--------- | :----------------------------------------------------------------------------------------------- | :----- | :-------------------------------------------------------------------------------------------------------------------- | :--: |
| HAD-A1 | `empty`    | `empty-root.vue` 仅为带 `data-vean-empty-root` 的 `div`；Compact 拼 icon/title/description       | R1、R4 | **v0.50.0 W2 下沉 UI-only**：删除 aria 家族，Empty 由 UI 组合 Primitive/Icon 自持装配；排期前冻结，禁止作为新家族模板 |  📋  |
...
| ID     | 组件    | 证据                                                                                                    | 违反                  | 处置                                                                                                                          | 状态 |
| :----- | :------ | :------------------------------------------------------------------------------------------------------ | :-------------------- | :---------------------------------------------------------------------------------------------------------------------------- | :--: |
| HAD-B1 | `badge` | `badge-root.vue`：`useControllableState(open)` + 一个 `div`                                             | R4 临界；当模板则违反 | **v0.50.0 W2 下沉 UI-only**：可关闭需求组合 Button/Icon，不开 Badge aria 家族；排期前冻结槽位                                 |  📋  |
...
| HAD-B3 | `tag`   | `tag.vue` 仅 `open` + `close`；`defineOptions({ name: 'STag' })` 还把 UI 前缀写进了 aria                | R4 临界               | **v0.50.0 W2 下沉 UI-only**：可关闭标签交互以 `tags-input` 为准；同步修正 aria 组件名泄漏（`STag` → `Tag`，排期前禁止当模板） |  📋  |
...
| 组件                                                        | 为什么留下 / 现状                                                                                                                                                                                                                                        |
| :---------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `drawer`                                                    | 现状无独立 aria 家族，UI 包装 `DialogCompact`——**v0.50.0 起决策反转为独立 aria 家族**（snap points / swipe / handle 独立状态机，[v0.50.0.md §4.1](./v0.50.0.md#41-drawer-独立-aria-家族--bottomsheet-退役)）；过渡形态的 compose Dialog 不再作为 R6 范例 |
...
| CMP-6.2 | 桶 B：Badge/Tag 下沉 UI-only；Alert 补 `role="alert"/"status"`；修正 aria 组件名 `STag` → `Tag`      |          v0.50.0 W2（0.5d）          | —                  |
...
| CMP-6.5 | HAD-C4 Card 家族退役：删 aria/card，SCard 改 UI-only 组合 Collapsible（breaking，不做兼容层）        |          v0.50.0 W2（1–2d）          | 0.x breaking 窗口  |
