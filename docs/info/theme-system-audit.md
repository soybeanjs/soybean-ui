...
> 状态：📄 审计报告（问题清单为结论，未排期；P0 项为可独立提交的修复）
> 基线：2026-09-18 · 分支 `vean` · `@vean/theme@0.50.0-beta.1`
> 方法：源码精读 + 引擎实测（本文所有数字均由附录 A 的脚本在本地跑出，非估算）+ 官方文档/源码检索（引用 URL 见附录 C）
>
> **后续：** 基于本报告的重设计提案见 [theme.md](../theme.md)（分层参照 / 完整 token 清单 / 对比度护栏 / 迁移阶段）。本报告为审计快照，落盘后不再更新。
...
→ 修复（三选一）：(a) `sidebarDerive: false` 时仍输出变量、值等于 base 对应 token（推荐，与文档一致）；(b) 让 unocss 的映射带 fallback（`var(--sidebar, var(--background))`）——注意 `colors.ts:16-22` 目前对 hsl 是 `hsl(var(--x))`，无法表达"整条 fallback"，需要改成 `hsl(var(--sidebar, var(--background)))`；(c) 删除该开关。
...
| `--vean-*` 组件级变量                   | 28 个（全部是布局/测量值，无视觉 token）                          |
