...
| `toggle-group`                                | 泛型 SFC 字面量 Boolean 默认值丢失（`collapsible`/`unmountOnHide`/`clearable` 等） | Vue 编译行为      | [C42](./check-reports/C42-toggle-group.md)                                    |
...
| C42  | `toggle-group` | [C42-toggle-group.md](./check-reports/C42-toggle-group.md) | **Major ×1**（D1-08/D7-11 编译行为）：泛型 SFC 中 `withDefaults` 的**字面量 Boolean 默认值**（`rovingFocus: true` 等）被编译器静默丢弃并 cast 为 `false`，导致 roving-focus 导航/loop 循环/clearable 去选三功能静默失效 → 改**函数式默认值**（`rovingFocus: () => true`）+ 运行时 `??` 回退，UI 透传层同步声明默认值（否则 Boolean cast 的 `false` 会覆盖子组件默认）。**同缺陷波及** `accordion-root`/`accordion-compact`（`collapsible`/`unmountOnHide`）、`select-compact`（`clearable`），已列入遗留排期。**文档** en/zh 4 节 → 8 节 Recommended structure（含 Component family + 12 能力 × 6 库对标表） | 8 → 24          |

> **批次 2 合计：** 3 个检查单元通过 D1–D7 全维度；单测 19 → 69 项（+50）；`pnpm typecheck` / `pnpm lint` 全绿；族系回归（toggle-group/slider/switch/checkbox/radio-group/toolbar 等）全通过。
