...
| C90  | `rating`            | 多槽           | ✅  | ✅  | ✅  | ✅  | ✅  | ✅  | ✅  |   P1   | D1-08, D2-11, D3-01, D7-05                      |
...
### 4.13 批次 12（P1 数据展示）— 2026-08-13

覆盖第 10 轮（P1 数据展示）的 `carousel`（C66），延续 D1–D7 全维度审计；本轮修复 headless 层内联视觉隐藏类边界违规与默认可访问文案硬编码英文两处缺陷，并将单测从 6 项扩展到 7 项。

| 编号 | 组件       | 检查报告                                           | 缺陷修复（按严重度）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 单测（前 → 后） |
| :--: | :--------- | :------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------- |
| C66  | `carousel` | [C66-carousel.md](./check-reports/C66-carousel.md) | **Major ×1**（D1-02 边界违规）：`carousel-previous.vue`/`carousel-next.vue` 默认插槽用 `<span class="soybean-headless-sr-only">` 承载屏幕阅读器文本——headless 层内联视觉隐藏类（依赖 `config-provider` 注入的全局样式），违反「headless 零样式（连 `hidden`/`sr-only` 都不行）」硬约束 → 改用既有 `VisuallyHidden` 原语（`feature="fully-hidden"`）。**Major ×1**（D2-11 硬编码英文）：三处默认可访问文案硬编码——根区域 `aria-label` 回退 `'Carousel'`（且 `as string` 显式断言）、previous `'Previous slide'`、next `'Next slide'` → 新增 `LocaleCarouselMessages { previous, next, ariaLabel }` 并同步 13 语言包，经 `useLocaleMessages` 本地化回退并移除断言。**Minor ×1**（D1-15 装饰元素）：默认 `Icon` 缺 `aria-hidden` → 补 `:aria-hidden="true"`。**文档** en/zh 4 节 → 9 节 Recommended structure（含 Component family + 7 能力 × 6 库对标表 + Cautions 6 条 + FAQ 6 组） | 6 → 7           |

> **批次 12 合计：** 1 个检查单元通过 D1–D7 全维度；单测 6 → 7 项（+1）；`pnpm typecheck` / `pnpm lint` 全绿；本地化改造经全量单测（1680 项）回归无副作用。批次 12 完结，后续按执行顺序进入下一检查项。

### 4.14 批次 13（P1 表单补充）— 2026-08-13

覆盖第 9 轮（P1 表单补充）的 `rating`（C90），为任务列表最后一个检查单元。`rating` 为新近发布组件，本轮首次执行 D1–D7 全维度审计。

| 编号 | 组件     | 检查报告                                       | 缺陷修复（按严重度）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 单测（前 → 后） |
| :--: | :------- | :--------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------- |
| C90  | `rating` | [C90-rating.md](./check-reports/C90-rating.md) | **无 Blocker/Major**。headless `RatingRoot` + `RatingItem` 多槽模式：`useControllableState` 受控/非受控、`role="slider"` 完整 ARIA、ArrowKeys/Home/End 键盘导航（`allowHalf` 0.5 步进）、RTL（`useDirection` + `isRatingBackKey` 三向键映射）、`allowClear` 清除、`readonly`/`disabled` 守卫、表单集成（`VisuallyHiddenInput`）；5 纯函数下沉 `shared.ts`；UI 层 `ratingVariants` + `ratingItemVariants` 薄包装 + `clip-path` 半星裁剪。**文档** 新建 en/zh 7 节（Overview/Usage/Features/Demos/API/Notes/FAQ + 11 能力 × 6 库对标表 + 4 Cautions + 4 FAQ） | 13 → 33         |

> **批次 13 合计：** 1 个检查单元通过 D1–D7 全维度；单测 13 → 33 项（+20）；`pnpm typecheck` 全绿（仅既有 `ImportMeta.env` 基线）；en/zh 文档新建。**至此 C01–C90 全部 90 个检查单元完成 D1–D7 全维度审计。**
...
_本项目快照覆盖 88 个已发布组件 × 7 大维度 × 105 个检查项（方法论见 audit.md）。最后更新：2026-08-13。C01–C90 全部 90 个检查单元已完成 D1–D7 全维度审计（批次进度与记录见 [四、组件检查执行记录](#四组件检查执行记录)）。遗留增强项见 4.2 节，非阻塞。_
