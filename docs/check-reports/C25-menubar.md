...
- **e2e 基线既有破损（已修复）**：vitest 升级至 4.1.10 后 `toBeFocused` matcher 被移除（改名 `toHaveFocus`），[dialog.e2e.spec.ts](file:///Users/soybean/Web/Projects/SoybeanJS/soybean-ui/packages/ui/test/browser/specs/components/dialog.e2e.spec.ts) 与 [select.e2e.spec.ts](file:///Users/soybean/Web/Projects/SoybeanJS/soybean-ui/packages/ui/test/browser/specs/components/select.e2e.spec.ts) 曾因此失败。已于本次检查期间修复并提交（`351fde40`）：`toBeFocused` → `toHaveFocus`、body 扫描豁免页面级 `region`。复核（2026-08-02）：dialog 3 项 + select 3 项通过，全量 e2e 基线 5 文件 19 项全绿，无 `toBeFocused` 残留。
