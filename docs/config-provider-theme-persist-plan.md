...
- `@soybeanjs/theme` 已具备存储基础能力：`THEME_STORAGE_KEY`（`__SOYBEAN_THEME`）、`stringifyThemeConfig` / `parseThemeConfig`（含键枚举校验与未知字段忽略）、`get/set/removeStoredThemeConfig`（localStorage，SSR-safe）、`createThemeInitScript`（head 内联脚本，首帧前从 localStorage 应用持久化主题并设 `data-theme` / dark class）。主题**只存 localStorage**，不再使用 cookie 持久化或传输。
- 现状缺口：
  1. 持久化读取发生在**应用层**（`apps/nuxt/app.vue` 读 localStorage、`apps/playground/src/theme.ts` 用 `useStorage`），ConfigProvider 只消费显式 `theme` props，能力未下沉、每渲染重复反序列化。
...
2. **存储配置**（SSR 场景：`themeConfig` prop 注入；客户端：localStorage），仅补齐未显式声明的键；
...
| ② SSR 首帧主题应用   | `themeConfig` prop 注入 + `createThemeInitScript` 内联脚本 | SSR 首帧渲染默认主题，内联脚本首帧前从 localStorage 应用 |
| ③ 自定义 preset 存储 | `presetStorage` prop + `@soybeanjs/theme` 既有 API         | 本地存储为权威源，SSR 走注册表注入                       |
...
export interface ConfigProviderProps extends _ConfigProviderProps {
   ...
   *
   * @defaultValue THEME_STORAGE_KEY（'__SOYBEAN_THEME'）
   */
  themeStorageKey?: string;
  /**
   * SSR 注入的持久化配置（服务端渲染时穿透给客户端）。
   *
...
- **写回同步**：`commitPersistedConfig(config)` 内 `setStoredThemeConfig` 后立即更新 `persistedConfig` 并复位脏标记，使下游 `createTheme` 无需重读存储。
...
## 4. 功能二：SSR 首帧主题应用（localStorage + createThemeInitScript）

主题只存 localStorage，不再使用 cookie 作为跨环境同步通道。SSR 模式下，服务端首帧渲染**默认主题**；`createThemeInitScript()` 生成的 head 内联脚本在**客户端首帧绘制前**直接从 localStorage 读取持久化主题，设置 `data-theme` 与 dark class，避免 FOUC。颜色 CSS 由客户端 `createTheme` 注入。

### 4.1 存储键与首帧脚本约定

| 项       | 规范                                     | 说明                                                          |
| -------- | ---------------------------------------- | ------------------------------------------------------------- |
| 存储键   | `__SOYBEAN_THEME`（`THEME_STORAGE_KEY`） | 与 `setStoredThemeConfig` 使用同一键，内联脚本读取无需改键    |
| 命名规则 | `__SOYBEAN_*` 前缀                       | 与引擎其他存储键一致，避免与其他库冲突                        |
| 定制     | `createThemeInitScript({ storageKey })`  | 应用可整体替换，需与 `setStoredThemeConfig` 的键保持一致      |
| 值格式   | `JSON.stringify(ThemeConfigState)`       | localStorage 无 4KB 上限，主题配置 JSON 约 <200B，余量充足    |
| 安全     | 仅颜色/尺寸偏好、无敏感信息              | 由 `createThemeInitScript` 统一保证，不经过 cookie 请求头传输 |

### 4.2 应用时机

| 时机             | 动作                                                                                  | 位置                                 |
| ---------------- | ------------------------------------------------------------------------------------- | ------------------------------------ |
| 服务端首帧渲染   | 渲染**默认主题**（不读取任何客户端持久化源）                                          | 服务端组件树渲染                     |
| 客户端首帧绘制前 | `createThemeInitScript()` 读 localStorage → 设 `data-theme` / dark class，不写 cookie | head 内联脚本（既有能力）            |
| 运行时写回       | `commitPersistedConfig` → `setStoredThemeConfig` 写 localStorage                      | ConfigProvider（新增）               |
| 可选 SSR 透传    | 应用注入 `themeConfig` prop，使服务端 HTML 与持久化主题一致（配合 preset 注册表）     | Nuxt `app.vue` setup 或 nitro 中间件 |

**一致性链路**：客户端首帧前内联脚本从 localStorage 应用主题；SSR 首帧渲染默认主题，若应用注入 `themeConfig` 则服务端渲染该主题、客户端水合复用。两次首帧之间无主题闪烁。

### 4.3 异常处理机制

| 异常                                | 处理                                          | 位置                             |
| ----------------------------------- | --------------------------------------------- | -------------------------------- |
| localStorage 缺失 / 空值            | `raw` 为空 → 跳过，保持默认主题               | `createThemeInitScript`          |
| `data-theme` / class 应用失败       | 内联脚本整体包 `try/catch` → 静默回退默认主题 | `createThemeInitScript`          |
...
- `persistTheme = true` 且 SSR 时：`readPersistedConfig()` 优先取 `props.themeConfig`（应用层注入），客户端则取 localStorage；
- 服务端渲染的 CSS（内联 `<style id="__SoybeanUI_theme">`）基于合并后的有效配置生成，与客户端首帧一致；
- **不新增框架依赖**：ConfigProvider 保持框架无关，首帧主题应用由 `createThemeInitScript` 内联脚本完成，其他 SSR 框架自行在 `<head>` 注入该脚本即可。

## 5. 功能三：自定义 preset 存储

### 5.1 存储位置与格式

- **存储位置**：localStorage（客户端权威源）。主题全程不写 cookie，preset 定义同样只存 localStorage，不经请求头传输。
- **存储键**：`__SOYBEAN_THEME_PRESETS`（新常量 `THEME_PRESETS_STORAGE_KEY`）。
- **格式规范**（JSON）：
...
- **服务端注册表**：应用把与 localStorage 同源的 preset 定义注册到服务端（构建时从 `@soybeanjs/theme-presets` 数据包导入，或服务端自定义注册），通过新增 `presetProvider` 注入 ConfigProvider → SSR HTML 完整渲染自定义 preset，**无闪烁**。已应用 preset 的引用存于 localStorage（`APPLIED_PRESET_KEY` = `__SOYBEAN_THEME_APPLIED_PRESET`），不写 cookie；SSR 经 `presetProvider` 解析，客户端直读本地 presets 表。
...
3. **`@soybeanjs/ui` ConfigProvider**：
   - `types.ts` 新增 `persistTheme` / `cacheThemeConfig` / `themeStorageKey` / `themeConfig` / `presetProvider`；
...
4. **`apps/nuxt` 落地**：`app.vue` 注入 `createThemeInitScript` head 内联脚本，并透传 `:theme-config`；补充自定义 preset 保存/应用示例。
...
| 用例                                     | 断言                                      |
| ---------------------------------------- | ----------------------------------------- |
| 客户端写入 localStorage 主题后刷新首页   | 首帧 `data-theme` 与 CSS 变量与持久化一致 |
| 无持久化主题请求首页                     | 默认主题渲染，无 500                      |
| 保存自定义 preset 后刷新（服务端注册表） | 首帧即应用自定义 preset，无闪烁           |
...
## 9. SSR 环境判断与统一存储（localStorage-only）

### 9.1 背景：`import.meta.env.SSR` 固化问题

`@soybeanjs/theme` 与 UI 库均为**预构建产物**，构建时 `import.meta.env.SSR` 会被求值并固化为常量（通常为 `false`），因此**无法在消费方真实 SSR 运行时反映服务端环境**。为此统一使用**运行时检测** `isServerRuntime()`（`typeof window === 'undefined'`）判断服务端，应用也可通过 `isServer` prop 显式注入（如 Nuxt `import.meta.server`）。

### 9.2 统一存储（localStorage-only）

原 `createThemeStore` 门面（以 cookie 作为跨环境同步通道）已废弃：theme 配置、preset 表、已应用 preset 引用均**只存 localStorage**，不再读写 cookie。SSR 一致性由 `createThemeInitScript` 内联脚本在客户端首帧前从 localStorage 应用主题来保证，服务端首帧渲染默认主题、经 `themeConfig` / `presetProvider` 透传注入。相关逻辑由 `@soybeanjs/ui` ConfigProvider 的 `createThemeContext`（见 `use-theme.ts`）承载，不再依赖任何 cookie 头或 `useRequestHeaders`。

- 主题配置：`THEME_STORAGE_KEY`（`__SOYBEAN_THEME`）；
- 自定义 preset 表：`THEME_PRESETS_STORAGE_KEY`（`__SOYBEAN_THEME_PRESETS`）；
- 已应用 preset 引用：`APPLIED_PRESET_KEY`（`__SOYBEAN_THEME_APPLIED_PRESET`）；
- 首帧应用：`createThemeInitScript({ storageKey, darkSelector, setDataTheme })` 内联脚本，无 `config` 参数、不写 cookie。

### 9.3 环境限制

- **`isServer` 显式传**：库无法自判消费方环境，应用应传 `import.meta.server`（Nuxt）或等价 SSR 标志；不传时回退 `isServerRuntime()` 运行时检测。
- **写存储仅客户端生效**：服务端无 `localStorage` / `document`，写操作均为 no-op 或返回 `false`；SSR 首帧由内联脚本在客户端应用主题，无需服务端写任何存储。
