# Vean 品牌重命名方案：SoybeanUI → Vean

> 定位：把「品牌 + npm scope + 逻辑层名」三件事一次性换成 Vean 体系的**总方案**，含影响面量化、平滑过渡策略、执行编排与配套 codemod。给维护者与执行 Agent 看。
> 状态：💡 提案
...
| 决策项               | 选择                                                                                                                                                                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 品牌名               | **Vean**，来源 **V**ue + Soy**bean**（V = Vue，ean = Soybean 的延续）                                                                                                                                                                           |
| 逻辑层               | `@soybeanjs/headless` → **`@vean/aria`**（沿用 v0.50.0 已确定的 aria 命名，只换 scope）                                                                                                                                                         |
| 样式层               | `@soybeanjs/ui` → **`@vean/ui`**                                                                                                                                                                                                                |
| 主题 / UnoCSS / 技能 | `@soybeanjs/theme` → `@vean/theme`；`@soybeanjs/ui-uno` → `@vean/unocss`；`@soybeanjs/ui-skills` → `@vean/skills`                                                                                                                               |
| 消费者 CLI           | `sbean` → **`@vean/cli`** bin名称为vean；配置文件 `sbean.json` → `vean.json`                                                                                                                                                                    |
...
| 品牌混淆风险         | `vean` 与 `vben`（`vue-vben-admin`，约 28k–33k star，技术栈与目标用户重合）近似 —— 品牌书写统一用 `Vean`，见 [§7.5.1](#751-附带发现vean-与-vben-的品牌混淆风险)                                                                                 |
| 运行时契约改名       | `--soybean-*` → `--vean-*`、`data-soybean-*` → `data-vean-*`：**做**，但用 codemod 兜底（92 + 511 个符号）                                                                                                                                      |
...
| 域名                 | `ui.soybeanjs.cn` → **`veanui.com`**（canonical，**已注册持有**；`vean.dev` 未注册，不采用）；`vean-ui.com` 作可选 301 防御域；`vean.soybeanjs.cn` 作过渡入口 301，见 [§7.4](#74-域名与-registry-url)；旧域名路径保持型 301，并行服务 ≥ 12 个月 |
| 仓库                 | GitHub repo `soybeanjs/soybean-ui` → **`soybeanjs/vean`**（留在团队 org，带走 15k star 联动）；`vean-ui` 的 GitHub 名留作占位                                                                                                                   |
...
| 不变项                                                                                  | 理由                                                                                                                                                                                                                                                              |
| --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`S` 组件前缀**（`SButton`、`SDialog`）                                                | **`S` = Styled**——样式封装层标识，与 `@vean/aria` 的无前缀原语（`Button`）相对，同时也是让两层能在同一文件共存的机制。改前缀实测成本 **787 文件 / 4,984 处**（codemod 实测）+ 全部下游，而 `V` 还撞 Vuetify。详见 [§7.5](#75-组件前缀保留-s定义为-styled不改-v)。 |
...
| `@soybeanjs/cva` / `@soybeanjs/colord`                                                  | 通用工具包，与 UI 品牌无绑定关系，锁定在 `@soybeanjs` scope 可避免把 breaking 扩散到两个独立包。`@vean/ui` 依赖 `@soybeanjs/cva` 完全正常。                                                                                                                       |
...
| 现状             | 规模                                                         | 目标          | 处理                                                     |
| ---------------- | ------------------------------------------------------------ | ------------- | -------------------------------------------------------- |
| `data-soybean-*` | **511 个唯一属性**、1,539 处、579 个文件                     | `data-vean-*` | v0.50.0 一次改完；codemod 提供 `--runtime-contract` 开关 |
| `--soybean-*`    | **92 个唯一变量**、212 处（`theme` 包 0 处，全在组件作用域） | `--vean-*`    | 同上                                                     |
...
| 现状                             | 目标        | 规模（含文档/生成物） |
| -------------------------------- | ----------- | --------------------- |
| `SoybeanUI`                      | `Vean`      | 573 处（源码/文档）   |
| `SoybeanHeadless`                | `Vean Aria` | 21 处                 |
| `@soybeanjs/*`（文档正文引用）   | `@vean/*`   | 3,300+ 处             |
...
| `packages/cli`      | 59     | 原 `sbean` 目录（v0.50.0 Phase A 已迁移）→ `@vean/cli` CLI，含 registry / templates |
...
- 子路径必须逐个转发（`@soybeanjs/ui/button` → `@vean/ui/button`），`exports` map 要一一对应，否则 tree-shaking 与 standalone 导入会断。
- 墓碑版的 `peerDependencies` 指向 `@vean/ui@0.50.x`，避免用户停留在旧语义。
...
3. 跑 codemod repo profile（`node tools/vean-codemod/migrate.mjs . --profile=repo --write`）完成 A/B 层字符串替换。
4. `packages/scripts`：`catalog.ts` 的 `CatalogTarget = 'headless' | 'ui'` → `'aria' | 'ui'`，`srcDir` 改 `packages/aria/src`；`index.ts` 帮助文本同步。
5. `packages/cli`（原 `sbean`，目录已在 v0.50.0 Phase A 就位）：包名 `sbean` → `@vean/cli`、`bin.vean`、`REGISTRY_URL`/`DOCS_URL`/`GITHUB_SOURCE_URL`、`DEFAULT_REGISTRY_NAMESPACE = '@vean'`、配置文件 `sbean.json` → `vean.json`（schema 文件名跟随）。
6. Nuxt 模块：`meta.name` / `configKey` 从 `@soybeanjs/headless`、`@soybeanjs/ui` 改为 `@vean/aria`、`@vean/ui`（**这是用户 nuxt.config 里的键，必须写进升级指南**）。
...
配套脚本见 **`tools/vean-codemod/`**，零运行时依赖，Node 18+ 直接跑，默认 dry-run。
...
node tools/vean-codemod/migrate.mjs . --write --cli --new-domain=veanui.com

# 本仓库自改（额外处理品牌文案、仓库路径、内部标识）
node tools/vean-codemod/migrate.mjs . --profile=repo --write --new-domain=veanui.com --repo-slug=soybeanjs/vean
```
...
| **长匹配优先**       | `ui-uno` 规则排在 `ui` 之前，避免 `@soybeanjs/ui-uno` → `@vean/ui-uno`                                                         |
...
- **建议**：仍用 `@vean/aria`（v0.50.0 已论证该命名，且"aria 层"在 Base UI / Ark UI 语境下已泛指无样式行为层），但**在文档首段明确界定**："aria = 无样式行为层，包含 a11y 状态机、composable、日期与格式化基础能力"。
- **备选**：`@vean/pure` / `@vean/primitives`。若改名窗口内仍觉得别扭，此时切换成本最低——**这是唯一值得在动工前再确认一次的命名**。
- **风险**：`aria` 一词在 npm 上高度拥挤（`aria-*`、`@react-aria/*`），品牌识别度弱于 `vean`。缓解方式：文档、README、搜索关键词统一以 `@vean/aria` 整体出现。
...
- **对策**：升级指南用独立章节逐条列出；codemod 提供 `--runtime-contract`；在 v0.50.0 breaking note 中单独点名；文档站组件示例页给出 `data-vean-*` 的显式说明。

### 7.3 发布顺序错位

`@vean/ui` 依赖 `@vean/aria`，`aria` 又依赖 `@vean/theme`。若用 `workspace:^` 且发布乱序，消费者会装到指向未发布版本的 peer。
...
> | 版本           | 结论                     | 依据                             | 为何变更                                                                                                                 |
> | -------------- | ------------------------ | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
> | v1             | `vean.soybeanjs.cn` 过渡 | 无独立域                         | 否决：子域 → 独立域要再迁一轮 canonical/SEO（见下）                                                                      |
> | v2             | `vean-ui.com`            | "`vean-ui` 是三处唯一齐全的名字" | **依据失效**：它依赖"GitHub 拿不到 `vean`"，但仓库挂在 org `soybeanjs` 下，GitHub 端本就不需要 `vean` 这个名字           |
...
3. **命名不该窄于它承载的东西**（与 §7.1 同一条原则）：`vean-ui.com` 只点了 `@vean/ui` 一个包，但这个站要同时承载 `@vean/aria`（无样式层）、`@vean/theme`、`@vean/unocss` 与 `vean` CLI。`chakra-ui.com` / `mui.com` 能用 `-ui` 后缀，是因为它们**整个产品**就叫 Chakra UI / MUI；这里伞形产品叫 Vean，只有其中一个包叫 UI。
4. **`.dev` 预置 HSTS**（Google 注册局，TLD 级）：浏览器层面禁止 http 明文访问，免费拿到强制 HTTPS 与信任加成。**代价要说清**：该域不能用作本地开发别名（无法把 `vean.dev` 指向 localhost 起明文服务）。对本项目影响 ≈ 0——文档站与 registry 本来就必须跑 HTTPS。
5. **价格不构成差异**：`.dev` $12 vs `.com` $11，差 **$1/年**。选 `.com` 的理由不能是"省钱"，只能落在命名或受众上——而这两条都指向 `.dev`。
6. **`vean-ui.com` 的价值是防御，不是主力**：它挡的是 `vean-ui` / `veanui` 这条混淆与抢注路径。这是**主动付出的防御成本**，不是需要被"用掉"的沉没成本——所以它不构成"既然买了就用它"的理由。

**`.com` 什么时候才是更优解？** 只有一个场景：**决策人不是工程师**。企业采购、招投标、供应商名录、非技术买方瞥见 URL 的那一刻，`.com` 的信任溢价是真实的。但组件库的采购决策人是工程师，入口是 npm / GitHub / README —— `.com` 在这里的获客贡献接近 0。若将来 Vean 要做面向非技术买方的商业版（Enterprise 授权、私有部署报价），届时再谈 `vean.com`（它 2026-09-24 到期，可能掉落）也不迟。

**为什么不用 `vean.soybeanjs.cn` 做 canonical**（与选哪个独立域无关）：子域 → 独立域必然要再迁一轮 canonical / sitemap / 外部引用 / SEO 权重，而改名窗口的全部价值恰恰是"一次性把品牌信号立起来"。且二级域对工程师读者传递"还没定下来"的信号；soybean-admin 有 15k star，用二级域反而拉低预期。
...
**`vean.soybeanjs.cn` 仍然有用——只作过渡入口**：301 → `veanui.com`，成本 0，给中文存量用户一个落脚点，并把 `soybeanjs.cn` 的权重导向新站。

**推荐组合**

| 域名                | 用途                                                        | 年费              | 优先级  |
| ------------------- | ----------------------------------------------------------- | ----------------- | ------- |
| **`veanui.com`**    | **canonical**：主站 + 文档 + registry `/r/*` + npm homepage | **已注册持有**    | ✅ 已定 |
| `vean-ui.com`       | 301 防御域，挡 `vean-ui` 这条混淆与抢注路径                 | $11（未注册）     | 可选    |
| `vean.soybeanjs.cn` | 过渡入口，301 → `veanui.com`                                | $0                | 零成本  |
| `vean.dev`          | 未注册，不采用（v3 候选，购买计划未执行）                   | —                 | ❌      |
| `vean.com`          | 远期：等掉落，或等出现非技术买方时再谈                      | $27（第三方持有） | 暂不    |

**最小可行组合**：`veanui.com` 已持有，**$0 增量成本**；可选加固：`vean-ui.com` 防御域（+$11/年）。

**仓库与 org：继续留在 `soybeanjs`。** repo 改名 `soybean-ui` → **`vean`**（即 `soybeanjs/vean`），仍挂在团队 org 下——org 是团队资产，能带走 soybean-admin 的 15k star 联动与 trust 传递；`vean-ui` 那个 GitHub 名留作占位，可给未来的官网或示例仓库。codemod 对应 `--repo-slug=soybeanjs/vean`。

**技术侧：域名决策不阻塞迁移。** npm scope `@vean/*` 与域名无绑定关系，域名只影响 5 处：
...
- **codemod 已覆盖**：`--new-domain=veanui.com [--new-cdn=<host>] [--repo-slug=soybeanjs/vean]`。host 替换与包名替换同一轮完成，已验证幂等；`github.com/soybeanjs`（org 本身）与 R2 对象前缀**刻意保留**。
- **旧域名必须路径保持型 301**：`/r/*`、`/schema/*`、`/components/*.md`、`/llms*.txt` 逐路径保留。旧版 `sbean` 的 `REGISTRY_URL` 是硬编码的，跳首页会让全部存量 CLI 直接坏掉。新旧并行 ≥ 12 个月。
- **R2 对象路径前缀 `/soybeanjs/` 不改写**：它标识桶内对象，不搬对象就改 URL 会 404。正确做法是给现有 bucket 绑自定义域 `assets.veanui.com`（对象原地不动）；若要统一前缀，先复制对象再改引用。

### 7.5 组件前缀：保留 `S`，定义为 **Styled**（不改 `V`）

组件叫 `SButton`，品牌叫 Vean——"S 从何而来"必然成为反复被问的问题。三个选项都做过成本实测：
...
`@vean/aria` 导出无前缀原语（`Button`），`@vean/ui` 导出 `SButton`。前缀是让两层能在同一个文件里共存的东西：
...
**文案规范**：统一写作 **`S` = Styled，标识样式封装层，与 `@vean/aria` 的无前缀原语相对。** 不要写"S 原本是 Soybean"——一句自洽、可长期维持的解释，比一段改名沿革更适合出现在文档里。

> **codemod 已备好但建议不用。** `tools/vean-codemod/rename-components.mjs` 实现了 S→V 的全量重命名（白名单驱动，已验证误伤为零、幂等）。它的价值在于**把这个选项的成本量化清楚**（787 文件 / 4,984 处），让"不改"成为有依据的决策而不是拖延。若将来确实要改，一条命令即可：
>
> ```bash
> node tools/vean-codemod/rename-components.mjs . --write
> ```

#### 7.5.1 附带发现：vean 与 vben 的品牌混淆风险
...
| -------- | ----------------------------------------- | -------------------------------------------- |
| 编辑距离 | **1 个字符**                              |                                              |
| 代表项目 | `soybeanjs/vean`                          | `vbenjs/vue-vben-admin`，**约 28k–33k star** |
| 技术栈   | Vue3 + Shadcn + Vite + TS + pnpm monorepo | Vue3 + Shadcn + Vite + TS + pnpm monorepo    |
| 目标用户 | Vue 中后台开发者                          | Vue 中后台开发者                             |

技术栈与用户**完全重合**，而名字只差一个字母。

1. **口头与搜索混淆**：`vean` 极易被读错、打错成 `vben`；搜索引擎可能把 "vean" 纠错到 "vben"。
2. **npm 误装**：无 scope 的 `vben` 已被第三方占用（`vben@1.0.11`），`npm i vben` 装到的是别人的包。而本库真正要装的是 `@vean/ui`——scope 形式反而构成了隔离。
3. **前缀叠加**：若前缀也改 `V`，`VButton` + `Vean` 会进一步强化与 Vben 的视觉关联。**这是"不该改成 V 前缀"的第四条依据。**

**对策（暂定，待决策）**

- 品牌书写**始终用完整形态 `Vean`**（大写 B），避免全小写 `vean` 与 `vben` 在视觉上混淆。
- 官网 / README 首屏明确 **"Vean — Vue + Soybean"**，用来源叙事建立区分度（这本来就是 [§0](#0-结论先行) 里定下的品牌介绍方式）。
- npm 层已天然隔离（`@vean` scope vs 无 scope 的 `vben`），无需额外动作。
- 不建议为此再次改名——npm org、scope、CLI 包名、仓库与域名都已按 `vean` 铺开，改名成本远高于混淆风险。
