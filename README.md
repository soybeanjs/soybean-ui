# SoybeanHeadless

SoybeanHeadless is a collection unstyled components for Vue 3. It is designed to be lightweight and easy to use.

## Roadmap

### Components

✅ - / total: 110

| Priority | Name            | Status         | Alias                | 📝 Note               |
| -------- | --------------- | -------------- | -------------------- | --------------------- |
| 1        | Accordion       | ✅ Completed   |                      |                       |
| 1        | Alert           |                |                      |                       |
| 1        | AlertDialog     | ✅ Completed   |                      |                       |
| 1        | Arrow           | 🚫 No Need     |                      |                       |
| 1        | AspectRatio     | 🚧 In Progress |                      |                       |
| 1        | Avatar          | 🚧 In Progress |                      |                       |
| 1        | Badge           |                | Chip                 |                       |
| 1        | BottomSheet     |                | Drawer(shadcn-ui)    |                       |
| 1        | Breadcrumb      |                |                      |                       |
| 1        | Button          | ✅ Completed   |                      | support loading       |
| 1        | Calendar        |                |                      | v-calendar            |
| 1        | Card            | ✅ Completed   |                      |                       |
| 1        | Carousel        |                |                      |                       |
| 1        | Checkbox        | 🚧 In Progress |                      |                       |
| 1        | Collapsible     | ✅ Completed   |                      |                       |
| 1        | Combobox        |                |                      |                       |
| 1        | Command         |                |                      | support virtualized   |
| 1        | ConfigProvider  | ✅ Completed   |                      |                       |
| 1        | ContextMenu     | 🚧 In Progress |                      |                       |
| 1        | DataTable       |                |                      | support virtualized   |
| 1        | DateField       |                |                      |                       |
| 1        | DatePicker      |                |                      |                       |
| 1        | DateRangeField  |                |                      |                       |
| 1        | DateRangePicker |                |                      |                       |
| 1        | Dialog          | ✅ Completed   |                      |                       |
| 1        | Divider         |                | Separator            |                       |
| 1        | Drawer          |                | Sheet                |                       |
| 1        | DropdownMenu    | 🚧 In Progress |                      | support hover trigger |
| 1        | Editable        |                |                      |                       |
| 1        | Form            |                |                      |                       |
| 1        | HoverCard       |                |                      |                       |
| 1        | Icon            | ✅ Completed   |                      | based on iconify      |
| 1        | Input           |                |                      |                       |
| 1        | Kbd             |                |                      |                       |
| 1        | Label           | ✅ Completed   |                      |                       |
| 1        | Layout          |                |                      |                       |
| 1        | Link            | ✅ Completed   |                      |                       |
| 1        | List            |                |                      |                       |
| 1        | Listbox         |                |                      |                       |
| 1        | Menu            | 🚧 In Progress |                      |                       |
| 1        | Menubar         | 🚧 In Progress |                      |                       |
| 1        | NavigationMenu  | 🚧 In Progress |                      |                       |
| 1        | NumberInput     |                | NumberField          |                       |
| 1        | Pagination      |                |                      |                       |
| 1        | PinInput        |                | InputOPT, OPTInput   |                       |
| 1        | Popconfirm      |                |                      |                       |
| 1        | Popover         | 🚧 In Progress |                      |                       |
| 1        | Progress        |                |                      |                       |
| 1        | ProgressCircle  |                |                      |                       |
| 1        | RadioGroup      | 🚧 In Progress |                      |                       |
| 1        | RangeCalendar   |                |                      | v-calendar            |
| 1        | Resizable       |                |                      |                       |
| 1        | ScrollArea      |                |                      |                       |
| 1        | Segment         |                |                      |                       |
| 1        | Select          |                |                      | support virtualized   |
| 1        | Skeleton        |                |                      |                       |
| 1        | Slider          |                |                      |                       |
| 1        | Splitter        |                |                      |                       |
| 1        | Stepper         |                |                      |                       |
| 1        | Switch          |                |                      |                       |
| 1        | Table           |                |                      |                       |
| 1        | Tabs            |                |                      |                       |
| 1        | Tag             |                | Badge(shadcn-ui)     |                       |
| 1        | TagsInput       |                |                      |                       |
| 1        | Textarea        |                |                      | support auto size     |
| 1        | TimeField       |                |                      |                       |
| 1        | Timeline        |                |                      | ui things             |
| 1        | TimePicker      |                |                      | element-plus          |
| 1        | Toast           |                | Sonner               |                       |
| 1        | Toggle          |                |                      |                       |
| 1        | ToggleGroup     |                |                      |                       |
| 1        | Toolbar         |                |                      |                       |
| 1        | Tooltip         | 🚧 In Progress |                      |                       |
| 1        | Tree            |                |                      | support virtualized   |
| 1        | TreeMenu        |                |                      |                       |
| 1        | Viewport        |                |                      |                       |
| 1        | VisuallyHidden  | ✅ Completed   |                      |                       |
| 2        | Affix           |                |                      |                       |
| 2        | Anchor          |                |                      |                       |
| 2        | AutoComplete    |                |                      | support virtualized   |
| 2        | Backtop         |                |                      |                       |
| 2        | Cascader        |                |                      | support virtualized   |
| 2        | CheckboxCard    |                |                      |                       |
| 2        | Clipboard       |                |                      |                       |
| 2        | Code            |                |                      |                       |
| 2        | ColorPicker     |                |                      |                       |
| 2        | Comment         |                |                      |                       |
| 2        | Countdown       |                |                      |                       |
| 2        | CurrencyInput   |                |                      |                       |
| 2        | Descriptions    |                |                      |                       |
| 2        | Ellipsis        |                |                      |                       |
| 2        | Empty           |                |                      |                       |
| 2        | Equation        |                |                      | based on katex        |
| 2        | InfiniteScroll  |                |                      |                       |
| 2        | Mention         |                |                      | element-plus          |
| 2        | Navbar          |                |                      |                       |
| 2        | NumberAnimation |                |                      | naive-ui              |
| 2        | PageTab         |                |                      |                       |
| 2        | QRCode          |                |                      |                       |
| 2        | RadioCard       |                |                      |                       |
| 2        | Rating          |                | Rate                 | element-plus          |
| 2        | Result          |                |                      |                       |
| 2        | Spinner         |                | Loader, Spin         | github ldrs           |
| 2        | Statistic       |                |                      |                       |
| 2        | Tour            |                |                      |                       |
| 2        | Transfer        |                |                      |                       |
| 2        | TreeSelect      |                |                      |                       |
| 2        | Typography      |                |                      | shadcn-ui             |
| 2        | Upload          |                | FileUpload, Dropfile |                       |
| 2        | Watermark       |                |                      |                       |

### Utilities

✅ 10 / total: 14

| Priority | Name                | Status       | 📝 Note        |
| -------- | ------------------- | ------------ | -------------- |
| 1        | Popper              | ✅ Completed |                |
| 1        | Portal              | ✅ Completed | alias Teleport |
| 1        | Primitive           | ✅ Completed |                |
| 1        | RovingFocus         | ✅ Completed |                |
| 1        | Slot                | ✅ Completed |                |
| 1        | useCollection       | ✅ Completed |                |
| 1        | useDismissableLayer | ✅ Completed |                |
| 1        | useFocusGuards      | ✅ Completed |                |
| 1        | useFocusScope       | ✅ Completed |                |
| 1        | useMessage          |              |                |
| 1        | useModal            |              |                |
| 1        | useNotification     |              |                |
| 1        | usePresence         | ✅ Completed |                |
| 2        | useLoadingBar       |              |                |
